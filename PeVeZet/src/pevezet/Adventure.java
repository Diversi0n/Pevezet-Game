...
public class Adventure extends PApplet {
    ...
    private ArrayList<Plant> choosePlant, plants, combinePlant,plantpicker;
    ...
    private boolean shovelSelect, pickplant, plantFoodSelect,playlevel, cekclear, levelselect,ceklevelselect;
    private int playerSun, food, level,spawntime=20,zombieget,zombietotal;
...
//        pool = new GUIButton(740,250,280,210, new Color(255,255,0));shovel = new GUIButton(730,0,110,85, new Color(255,255,0, 0));
    ...
    public void setupLevel(){
        ...
        if(level>=1&&level<=5){
            ...
            // Level Modifier
            if(level==1){
                bg = loadImage("src/assets/Map/Grasswalklevel1.png");
            ...
            plantpicker.add(new Sunflower(choosePlant.get(0)));
            plantpicker.add(new Peashooter(choosePlant.get(1)));
            plantpicker.add(new Wallnut(choosePlant.get(4)));
            plantpicker.add(new PotatoMine(choosePlant.get(5)));
...
//                if(mouseX >= i.getBullet().getX() && mouseX <= i.getBullet().getX() + i.getBullet().getWidth() && mouseY >= i.getBullet().getY() && mouseY <= i.getBullet().getY() + i.getBullet().getWidth())
    ...
    public void levelSelect(){
        if(mouseX >= 88 && mouseX <= 1178 && mouseY >= 145 && mouseY <= 610) {
            int x = (mouseX - 88) / 235,  y = (mouseY - 145) / 260, idx = y * 5 + (x+1);
            if(mouseX > 233+(x*236) || mouseY > 345+(y*261))
                return;
            for(int i=1;i<=10;i++){
                if(i==idx){
                    level=idx;
    ...
    //        else if(mouseX >= 50 && mouseX <= 1210 && mouseY >= 100 && mouseY <= 680) {
                ...
                if(!tiles[y][x].hasPlant() && select != null && playerSun >= select.getPrice()&&tiles[y][x].isPlantable())
                {
    ...
    //                if(mouseX >= 89 && mouseX <= 463 && mouseY >= 205 && mouseY <= 533) {
                    ...
                    if(mouseX > 157+(x*102) || mouseY > 301+(y*116))
                        return;
                    for(Plant i : plants) {
                        if(i.getClass().equals(plantpicker.get(idx).getClass())) {
                            return;
                        }
                    }
                    if(plants.size() < 5) {
                        if(plantpicker.get(idx) instanceof Sunflower)
                            plants.add(new Sunflower(plantpicker.get(idx)));
                        else if(plantpicker.get(idx) instanceof Peashooter)
                            plants.add(new Peashooter(plantpicker.get(idx)));
                        else if(plantpicker.get(idx) instanceof Wallnut)
                            plants.add(new Wallnut(plantpicker.get(idx)));
                        else if(plantpicker.get(idx) instanceof PotatoMine)
                            plants.add(new PotatoMine(plantpicker.get(idx)));
                        else if(plantpicker.get(idx) instanceof SnowPea)
                            plants.add(new SnowPea(plantpicker.get(idx)));
                        else if(plantpicker.get(idx) instanceof Lilypad)
                            plants.add(new Lilypad(plantpicker.get(idx)));
                        else if(plantpicker.get(idx) instanceof TangleKelp)
                            plants.add(new TangleKelp(plantpicker.get(idx)));
                        else if(plantpicker.get(idx) instanceof CherryBomb)
                            plants.add(new CherryBomb(plantpicker.get(idx)));
                        else if(plantpicker.get(idx) instanceof Torchwood)
                            plants.add(new Torchwood(plantpicker.get(idx)));
                        else if(plantpicker.get(idx) instanceof CoconutCannon)
                            plants.add(new CoconutCannon(plantpicker.get(idx)));
                    }
                }
